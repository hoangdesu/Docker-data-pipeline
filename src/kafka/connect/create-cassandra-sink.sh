...
curl -s \
  ...
  "config": {
    ...
    "contactPoints": "cassandradb",
    "loadBalancing.localDc": "datacenter1",
    "topic.faker.kafkapipeline.fakerdata.mapping": "firstname=value.firstname, lastname=value.lastname, age=value.age, ssn=value.ssn, email=value.email, phonenumber=value.phonenumber address=value.address, job=value.job, company=value.company, year=value.year",
...
curl -s \
  ...
  "config": {
    ...
    "contactPoints": "cassandradb",
    "loadBalancing.localDc": "datacenter1",
    "topic.faker.kafkapipeline.fakerdata.mapping": "id=value.id, name=value.name, type=value.type, weight=value.weight, height=value.height",
    "topic.faker.kafkapipeline.fakerdata.consistencyLevel": "LOCAL_QUORUM"
  }
}'
echo "Done."